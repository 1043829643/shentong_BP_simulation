...
- `英雄池BP界面` 中我方/对方英雄池的每个位置不再展示 `S/A/B/C` 分级，而是按频次从高到低平铺展示；频次顺序来自原始英雄池数据中的先后位置，预置战队池按 `S → A → B → C` 合并后保留组内顺序。
...
- 当前先选方 Ban 手为 `1, 2, 5, 10, 11, 19, 21`，Pick 手为 `8, 14, 15, 18, 23`。
- 当前后选方 Ban 手为 `3, 4, 6, 7, 12, 20, 22`，Pick 手为 `9, 13, 16, 17, 24`。
- `getDraftTurn()` 根据当前步骤、我方阵营、先后手计算执行方。
- `selectHeroInPlan()` 在当前方案中追加一步 Ban/Pick。
- 已被当前方案 Ban/Pick 的英雄不可再次选择。
- 达到 24 手后不会继续追加新步骤。
- BP 页右侧时间线会预渲染全部 24 手的空槽位；执行 Ban/Pick 后对应槽位填入英雄头像。
- 当前等待操作的手数会高亮对应空槽位，并使用轻微缩放呼吸动画提示下一步落点。
...
- `buildPoolFromRaw()` 将原始分层数据转换成按频次排序的扁平 `HeroPool`，每个位置使用 `flat` 列表而不是 `S/A/B/C` 分组。
- `TEAM_POOL_ROWS` 支持内置按频次排序的战队英雄池；当前 `TRE` 使用 `TRE英雄池.xlsx` 的 `英雄池汇总` 口径写入预设池，且已排除 `使用 = 0` 的英雄。
- `flattenRolePoolByFrequency()` / `flattenTeamPoolByFrequency()` 用于兼容旧自定义英雄池数据，并保证我方/对方英雄池统一按频次展示。
...
- `src/types.ts`
- `resources/team-pools/TRE英雄池.xlsx`
...
- 支持 `.xlsx`、`.xls`、`.csv` 文件。
- Excel 使用 `xlsx` 解析；优先读取名为 `英雄池汇总` 的工作表，没有该工作表时读取首个工作表。
- CSV 使用 `papaparse` 解析。
- 新版默认英雄池格式为 TRE 宽表：每个位置一行，字段为 `位置`、`英雄1`、`频次1`、`使用1`、`被对手Ban1`、`英雄2`、`频次2`...；导入时按 `频次` 从高到低生成每个位置的英雄列表。
- 宽表导入时，如果某个英雄在某个位置 `频次 > 0` 但 `使用 = 0`，表示它只来自对手 Ban 记录，不视为该位置会玩的英雄，因此不会出现在该位置英雄池。
- 宽表中的摇摆位由同一英雄出现在 2 个及以上位置自动生成，排序按跨位置频次合计。
- 支持字段别名：
  - 英雄名：`英雄名`、`英雄`、`hero`、`heroName`、`name`
  - 常用位置：`常用位置`、`位置`、`role`、`position`、`pos`
  - 频次：`频次`、`frequency`、`count`
  - 旧长表优先级：`优先级`、`评级`、`tier`、`priority`
- 常用位置必须为 `1` 到 `5`。
...
- 默认初始配置为我方 `Team TRE`、对方 `Vici Gaming`、天辉、先选。
...
- XG、VG 有明确预置池。
- TRE 已接入真实频次预置池，来源为 `TRE英雄池.xlsx` 的 `英雄池汇总`。
- 其他战队目前走生成池逻辑。
